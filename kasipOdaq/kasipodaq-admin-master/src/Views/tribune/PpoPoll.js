...
class PpoPoll extends Component {
    ...
    changeTabCallback = (tab) => {
        switch (tab) {
            ...
            case '2':
                this.props.history.push({
                    pathname: `/tribune/ppo/poll/${this.props.match.params.id}`,
                    state: { tabId: 2,
    ...
    prevPage(){
        this.setState({ preloader: true })
        if (this.props.pollStore.testPageNumber > 1){
            this.props.pollStore.testPageNumber = this.props.pollStore.testPageNumber - 1
    ...
    nextPage(){
        this.setState({ preloader: true })
        if (this.props.pollStore.testPageNumber < this.props.pollStore.testPageCount){
            this.props.pollStore.testPageNumber = this.props.pollStore.testPageNumber + 1
                    ...
                    <TabsLayout tabs={this.state.tabs}
                        ...
                        <div className='legal-block'>
                            <div className="toggle-lang">
                                <div className={('lang ru ') + (this.state.currentTest ? 'active-tab' : '') } onClick={() => { this.setState({ archiveTest: false, currentTest: true }) }}>{this.props.userStore.languageList["Текущие опрос"] || 'Текущие опросы'}</div>
...

                            {
                                ...
                                <div className='legal-wrapper'>
                                    <Search currentPage={this.props.pollStore.testCurrentPage}
                                            pageCount={this.props.pollStore.testPageCount}
                                    ...
                                    <table>
                                        ...
                                        {
                                            this.props.pollStore.pollList.map((test, index) => {
...

                            {
                                ...
                                <div className='legal-wrapper'>
                                    <Search
                                        currentPage={this.props.pollStore.testCurrentPage}
                                        pageCount={this.props.pollStore.testPageCount}
                                        prevPage={this.prevPage}
                                        nextPage={this.nextPage}
                                        search={this.searchPoll}
                                    ...
                                    <table>
                                        ...
                                        <tbody className="list">
                                        {
                                            this.props.pollStore.pollArchiveList.map((test, index) => {
                                                return <tr className={'shown'} key={index}>
                                                    <td className="subject">
                                                        <div className={'legal-subject'}>
                                                            <Link to={'#'} className={'legal-link'}>
...
export default inject('pollStore', 'permissionsStore', 'unionStore', 'userStore')(observer(PpoPoll));
